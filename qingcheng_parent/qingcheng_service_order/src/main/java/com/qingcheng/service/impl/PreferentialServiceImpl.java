...
public class PreferentialServiceImpl implements PreferentialService {
    ...
    public int findPreMoneyByCategoryId(Integer categoryId, int money) {
        ...
        criteria.andEqualTo("categoryId", categoryId);//品类Id
        ...
        List<Preferential> preferentials = preferentialMapper.selectByExample(example);//优惠条例集合

        if (preferentials.size() >= 1) {//有优惠
            Preferential preferential = preferentials.get(0);//获取最佳优惠
            if ("1".equals(preferential.getType())) {//不翻倍
                return preferential.getPreMoney();//返回优惠金额
            }else {//优惠翻倍
                int multiple= money/preferentials.get(0).getBuyMoney();//翻倍倍数
