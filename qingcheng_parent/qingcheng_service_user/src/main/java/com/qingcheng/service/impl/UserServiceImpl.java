...
public class UserServiceImpl implements UserService {
    ...
    public void sendSms(String phone) {
        ...
        map.put("phone",phone);
        map.put("code",code+"");
        rabbitTemplate.convertAndSend("","queue.sms", JSON.toJSONString(map));  //转为字符串发给rabbit
    ...
    public void add(User user, String smsCode) {
        // 1 验证用户
        //比较短信验证码
        String code = (String) redisTemplate.boundValueOps("code_" + user.getPhone()).get();//缓存中的验证码
        if(code==null){
        ...
        if(userMapper.selectCount(searchUser)>1){
