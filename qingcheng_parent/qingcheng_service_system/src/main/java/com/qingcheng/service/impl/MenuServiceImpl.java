...
public class MenuServiceImpl implements MenuService {
    ...
    public List<Map> findAllMenu() {
        List<Menu> menuList = findAll();//查询到所有菜单
    ...
    public List<Map> findMenuByLoginName(String loginName) {
        List<Menu> menus = menuMapper.findMenuByLoginName(loginName);
        List<Map> mapList = findMenuListByParentId(menus, "0");
        return mapList;
    }
     ...
     * 根据父ID查询所有子菜单内容
     */
    public List<Map> findMenuListByParentId(List<Menu> menuList, String id) {
        List<Map> mapList = new ArrayList<Map>();
        for (Menu menu : menuList) {  //循环遍历一级菜单得到每个对应的子菜单
